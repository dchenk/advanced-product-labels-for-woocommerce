...
class BeRocket_products_label extends BeRocket_Framework {
    ...
    public $info, $defaults, $values, $templates;
    public $custom_post;
    ...
    public function init () {
        ...
        wp_register_style('berocket_products_label_templates_style',plugins_url( 'css/templates.css', __FILE__ ),"",
            $this->info[ 'version' ]
        );
    ...
    public function section_templates( $item, $options ) {
        if ( empty( $item['value'] ) ) $item['value'] = 'css-1';
    ...
    private function get_templates_section_html( $current_template = 'css-1' ) {
        $i = 1;
        $html = '';
