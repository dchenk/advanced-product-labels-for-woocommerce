...
function berocket_regenerate_tooltip() {
    jQuery('.br_alabel .br_tooltip').each(function() {
        if( ! jQuery(this).is('.br_tooltip_ready') ) {
            ...
            var data_trigger = jQuery(this).parents('.br_alabel').first().data('tippy-trigger');
            if( data_trigger ) {
                jQuery(this).parents('.br_alabel').first().on(data_trigger, function(event){event.preventDefault()})
            }
            tippy('.br_alabel_tooltip_'+br_tooltip_number+' span', {html:'.br_tooltip_'+br_tooltip_number});
...
jQuery(document).on('berocket_ajax_products_loaded berocket_ajax_products_infinite_loaded', berocket_regenerate_tooltip);
